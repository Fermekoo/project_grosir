...
$exe=mysqli_query($koneksi,$sql);
    ...
    if (!$ketemu){
       ...
       $sql_0="INSERT INTO return_barang (  id_pelanggan,id_barangtoko,jumlah_barang,harga,id_sesion) 
       VALUES ('$id_pelanggan','$id','1','$hrg','$sid')";
    ...
    //  $exe_ker=mysqli_query($koneksi,$sql_ker);
	 ...
	 // $exe_ub=mysqli_query($koneksi,$sql_ub);
        ...
        $sql_0u="UPDATE return_Barang
                SET jumlah_barang= jumlah_barang WHERE id_sesion ='$sid' AND id_barangtoko='$id'";
   ...
   // header('Location:penjualan.php');
	...
	echo"<div class='alert alert-danger'>
        ...
        <?php
        $sid = session_id();
          $sql_t="SELECT * FROM return_barang, stok_toko where id_sesion='$sid' AND return_barang.id_barangtoko=stok_toko.id_toko";
          $exe_t=mysqli_query($koneksi,$sql_t);
        

          while($data=mysqli_fetch_array($exe_t)){
             
            $subtotal = ($data['harga_return'])*($data['jumlah_barang']) ;
             $total += $subtotal;
            $harga_akhir = "Rp. ".number_format($data['harga_akhir'],'0',',','.')."-";
              $harga_return= "Rp. ".number_format($data['harga_return'],'0',',','.')."-";
        ...
        ?>
                ...
                <td><?php echo $data['tanggal'];?></td>
                 ...
                 <td><?php echo $harga; ?></td>
                   ...
                   <td><a href="#qty_modal" data-toggle="modal" data-target="#qty_dialog" data-id="<?php echo $data['id_return'];?>" data-jumlah="<?php echo $data['jumlah_barang'];?>" data-idtoko="<?php echo $data['id_barangtoko'];?>" data-hargaakhir="<?php echo $data['harga_return'];?>"><?php echo $data['jumlah_barang']; ?></a></td>

                   <td><a href="#harga_modal" data-toggle="modal" data-target="#harga_dialog" data-id="<?php echo $data['id_return'];?>" data-harga="<?php echo $data['harga_return'];?>" data-idtoko="<?php echo $data['id_barangtoko'];?>" data-jumlahker="<?php echo $data['jumlah_barang'];?>"><?php echo $harga_return; ?></a></td>

                  <td><?php echo $tampil_subtotal; ?></td>
          
          
          <td><a class="btn btn-danger" onclick="if (confirm('Apakah anda yakin ingin menghapus data ini ?')){ location.href='act_delete_return.php?id=<?php echo $data['id_return']; ?>' }"  class="glyphicon glyphicon-trash">Hapus</a></td>
    ...
    // NEGO HARGA
     $(document).ready(function () {
        $("#harga_form").on("submit", function(e) {
            var postData = $(this).serializeArray();
            var formURL = "act_returnbarang.php";
            $.ajax({
                url: formURL,
                type: "POST",
                data: postData,
...
<div class="row justify">
 <div class="col-sm-5 col-md-5">
  ...
  <li class="active"><a data-toggle="tab" href="#home">Ambil Uang</a></li>
  <li><a data-toggle="tab" href="#menu1">Simpan Uang</a></li>
...
<br>
     <form action="act_simpanuang.php" method="post">
  <div class="input-group input-group-lg">
